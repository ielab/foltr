...
class MqDataset:
    ...
    @classmethod
    def from_path(cls, root_path: str, n_features: int, n_folds: int, cache_root: Optional[str]) -> 'MqDataset':
        """
        ...
        """
        return MqDataset(root_path, MqDataset.read_mq(root_path, n_features, n_folds, cache_root))
    ...
    @staticmethod
    def read_mq(root_path: str, n_features: int, n_folds: int, cache_root: Optional[str]) -> List[FoldRecord]:
        """
        ...
        """
        folds = []
        for fold_dir in [f"{root_path}/Fold{i}" for i in range(1, n_folds+1)]:
