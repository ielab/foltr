...
n_features = 136
n_folds = 5
data = MqDataset.from_path("../data/MSLR-WEB10K/", n_features, n_folds, "../cache/")
...
def do_task(task):
    fold_id, click_model, ranker_generator_id, task_id = task
    # workaround pickle
...
tasks = itertools.product(range(len(data.folds)),
                          [NAVIGATIONAL_MODEL, INFORMATIONAL_MODEL, PERFECT_MODEL],
                          range(2))
tasks = list(tasks)
new_tasks = []
for idx, task in enumerate(tasks):
    new_tasks.append(list(task) + [idx])

n_cpu = min(3, len(tasks))
with Pool(n_cpu) as p:
    results = p.map(do_task, new_tasks)
...
np.save("../results/v0_mslr_foltr_results_50client_p1.0.npy", exp_results)
#
...
#
# plt.show()
# f.savefig(f'./results/v0_mslr_foltr_per_batch.png')
#
#
